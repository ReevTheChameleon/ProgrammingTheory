...
public class PlayerController : LoneMonoBehaviour<PlayerController>{
	...
	private void onInputZoom(InputAction.CallbackContext context){
		...
		vCam.GetCinemachineComponent<Cinemachine3rdPersonFollow>().CameraDistance =
			Mathf.Clamp(
				vCam.GetCinemachineComponent<Cinemachine3rdPersonFollow>().CameraDistance + delta,
				v2ZoomRange.x,
				v2ZoomRange.y
			)
		;
		//Cinemachine3rdPersonFollow followModule =
		//	vCam.GetCinemachineComponent<Cinemachine3rdPersonFollow>();
		//followModule.ShoulderOffset = followModule.ShoulderOffset.newZ(
		//	Mathf.Clamp(
		//		followModule.ShoulderOffset.z + delta,
		//		v2ZoomRange.x,
		//		v2ZoomRange.y
		//	)
		//);
